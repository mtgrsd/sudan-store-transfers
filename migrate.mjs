...
const steps = [
  // 1. Rename old tables to _old suffix (safe)
  `RENAME TABLE agents TO agents_old`,
  `RENAME TABLE agent_wallets TO agent_wallets_old`,
  `RENAME TABLE transfers TO transfers_old`,
  `RENAME TABLE customers TO customers_old`,
  `RENAME TABLE currencies TO currencies_old`,
  `RENAME TABLE company_wallet TO company_wallet_old`,
  `RENAME TABLE ledger_entries TO ledger_entries_old`,
  `RENAME TABLE transfer_confirmations TO transfer_confirmations_old`,
  // 2. Update users table
  `ALTER TABLE users MODIFY COLUMN role ENUM('admin','staff','agent') NOT NULL DEFAULT 'staff'`,
  `ALTER TABLE users ADD COLUMN IF NOT EXISTS phone varchar(64) DEFAULT NULL`,
  `ALTER TABLE users ADD COLUMN IF NOT EXISTS avatar text DEFAULT NULL`,
  `ALTER TABLE users ADD COLUMN IF NOT EXISTS login_method varchar(64) DEFAULT NULL`,
  `ALTER TABLE users ADD COLUMN IF NOT EXISTS is_active tinyint(1) NOT NULL DEFAULT 1`,
  `ALTER TABLE users ADD COLUMN IF NOT EXISTS last_signed_in timestamp NULL DEFAULT NULL`,
  // 3. Create new tables
  ...
  `INSERT IGNORE INTO system_settings (\`key\`, value, updated_at) VALUES
    ('receipt_validity_days', '7', UNIX_TIMESTAMP()*1000),
    ('company_name', 'متجر السودان', UNIX_TIMESTAMP()*1000),
    ('receipt_prefix', 'SUD', UNIX_TIMESTAMP()*1000)`,
];
