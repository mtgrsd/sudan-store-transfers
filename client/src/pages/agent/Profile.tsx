...
export default function AgentProfile() {
  ...
  const changePasswordMutation = trpc.user.changeMyPassword.useMutation({
    onSuccess: () => { toast.success("تم تغيير كلمة المرور بنجاح"); setShowPwChange(false); setPwForm({ current: "", next: "", confirm: "" }); },
  ...
  return (
    ...
    >
      ...
      <main style={{ maxWidth: "480px", margin: "0 auto", padding: "1rem" }}>
        ...
        {/* تغيير كلمة المرور */}
        <div style={{ backgroundColor: "white", borderRadius: "1.25rem", padding: "1.25rem", marginBottom: "1rem", boxShadow: "0 1px 6px rgba(0,0,0,0.06)" }}>
          <div style={{ fontSize: "0.875rem", fontWeight: "700", color: "#1a2e6b", marginBottom: "0.75rem" }}>
            🔑 كلمة المرور والأمان
          </div>
          {showPwChange ? (
            <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
              {[
                ...
                { label: "كلمة المرور الجديدة (6 أحرف+)", key: "next" },
                { label: "تأكيد كلمة المرور الجديدة", key: "confirm" },
              ].map(({ label, key }) => (
                <div key={key}>
                  <div style={{ fontSize: "0.75rem", color: "#374151", marginBottom: "0.3rem", fontWeight: "600" }}>{label}</div>
                  <input type="password" value={(pwForm as any)[key]}
                    onChange={(e) => setPwForm((f: any) => ({ ...f, [key]: e.target.value }))}
                    style={{ width: "100%", borderRadius: "0.625rem", border: "1.5px solid #e5e7eb", padding: "0.6rem 0.75rem", fontSize: "0.875rem", fontFamily: "monospace", boxSizing: "border-box" }} />
                </div>
              ))}
              {pwForm.next && pwForm.confirm && pwForm.next !== pwForm.confirm && (
                <div style={{ fontSize: "0.75rem", color: "#dc2626" }}>⚠️ كلمتا المرور غير متطابقتين</div>
              )}
              <div style={{ display: "flex", gap: "0.5rem" }}>
                <button onClick={() => setShowPwChange(false)}
                  style={{ flex: 1, background: "#f1f5f9", border: "none", borderRadius: "0.625rem", padding: "0.625rem", fontSize: "0.875rem", fontFamily: "Cairo, sans-serif", cursor: "pointer", color: "#374151" }}>
                  إلغاء
                </button>
                <button disabled={!pwForm.current || pwForm.next.length < 6 || pwForm.next !== pwForm.confirm || changePasswordMutation.isPending}
                  onClick={() => changePasswordMutation.mutate({ currentPassword: pwForm.current, newPassword: pwForm.next })}
                  style={{ flex: 1, background: "linear-gradient(135deg, #1a2e6b, #2563eb)", color: "white", border: "none", borderRadius: "0.625rem", padding: "0.625rem", fontSize: "0.875rem", fontWeight: "700", fontFamily: "Cairo, sans-serif", cursor: "pointer" }}>
                  {changePasswordMutation.isPending ? "⏳ جاري..." : "تغيير كلمة المرور"}
                </button>
              </div>
            </div>
          ) : (
            <button onClick={() => setShowPwChange(true)}
              style={{ width: "100%", background: "#f8fafc", border: "1.5px dashed #cbd5e1", borderRadius: "0.75rem", padding: "0.75rem", fontSize: "0.85rem", fontWeight: "600", fontFamily: "Cairo, sans-serif", cursor: "pointer", color: "#475569" }}>
              🔐 تغيير كلمة المرور
            </button>
          )}
        </div>
