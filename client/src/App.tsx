...
import AdminTransferNew from "./pages/admin/TransferNew";
import AdminTransferDetail from "./pages/admin/TransferDetail";
...
import AdminBalanceTransfer from "./pages/admin/BalanceTransfer";
import AdminReceiptTemplate from "./pages/admin/ReceiptTemplate";
...
function Router() {
  ...
  // super_admin و admin و employee → لوحة الإدارة
  // (تصحيح: الخادم يمنح employee صلاحيات قراءة/تشغيل مطابقة لـ admin في كل
  // الراوترات تقريبًا، لكن واجهة المستخدم كانت تُحوّل employee لواجهة الوكيل
  // فقط والتي تتطلب ربط حساب بمكتب — وهو غير متاح لـ employee، فتفشل الصفحة.)
  if (user.role === "super_admin" || user.role === "admin" || user.role === "employee") {
    return (
      <Switch>
        ...
        <Route path="/admin/transfers/new" component={AdminTransferNew} />
        ...
        <Route path="/admin/settings/whatsapp" component={AdminWhatsAppSettings} />
        <Route path="/admin/users" component={AdminUsers} />
        <Route path="/admin/balance-transfer" component={AdminBalanceTransfer} />
        <Route path="/admin/receipt-template" component={AdminReceiptTemplate} />
