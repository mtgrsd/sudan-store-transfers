...
const STATUS_LABELS: Record<string, { label: string; color: string; bg: string; icon: string }> = {
  ...
  received: { label: "مستلم", color: "#065f46", bg: "#d1fae5", icon: "✅" },
  cancelled: { label: "ملغاة", color: "#991b1b", bg: "#fee2e2", icon: "❌" },
  expired: { label: "منتهي الصلاحية", color: "#374151", bg: "#f3f4f6", icon: "⌛" },
};
...
export default function TransferReceipt({ transfer, onClose }: TransferReceiptProps) {
  ...
  const companyName = settings?.company_name || "متجر السودان";
  const companyPhone = settings?.company_phone || "";
  const footerText = settings?.receipt_footer_text || "شكراً لاستخدام خدماتنا";
  const primaryColor = settings?.brand_primary_color || "#1a2e6b";
  const logoUrl = settings?.company_logo_url || LOGO_URL;
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: "0.75rem", color: "#6b7280" }}>
              {companyPhone || "نظام التحويلات المالية"}
            </div>
            <div
              style={{
                marginTop: "0.75rem",
          ...
          >
            {[
              ...
              { label: "تاريخ الصرف", value: transfer.confirmedAt ? formatDate(transfer.confirmedAt) : "-" },
              ...(transfer.beneficiaryName ? [{ label: "اسم المستفيد", value: transfer.beneficiaryName }] : []),
              ...(transfer.beneficiaryPhone ? [{ label: "هاتف المستفيد", value: transfer.beneficiaryPhone }] : []),
              ...(transfer.beneficiaryId ? [{ label: "هوية المستفيد", value: transfer.beneficiaryId }] : []),
            ].map((item) => (
              <div
                key={item.label}
                style={{
